...
#### GPU 重构 / GPU refactor
> 重大架构变更，详见 `minecraft/GPU模块重构技术规划.md`（重构动机/目标架构/8 阶段计划/验收标准）与 `NetCraft/NetCraft.Gpu/Overview.md`。即时模式 → submission/render phase 分离架构（对标原版 26.2 Blaze3D）重构完成，规划阶段 0-6+8 已实现，阶段 7 部分完成（Tick 已独立 20tps 线程，Render 仍串行在 `_window.Run` 内）。GPU 子系统完成度 ~95%，文件数从 30 增至 ~120 / Major architectural change from immediate-mode to submission/render phase separation (mirrors vanilla 26.2 Blaze3D). Refactor plan stages 0-6+8 done, stage 7 partial (Tick on independent 20tps thread, Render still serial in `_window.Run`). GPU ~95% complete, file count grew from 30 to ~120.

- **阶段 0 解耦准备**：新增 `IGpuLogger` 抽象 + `GpuLogger` 默认实现，`VulkanGuiApp` 构造注入解耦 NetCraft.Logging；`NetCraft.Gpu.csproj` 移除 NetCraft.Util/Interop ProjectReference，零 NetCraft.* 依赖 / Decoupled from NetCraft.Logging via IGpuLogger; csproj drops NetCraft.Util/Interop refs
- **阶段 1 GPU 抽象 SPI 改造**：新增 `ICommandEncoder.cs` / `IRenderPass.cs` / `PipelineCache.cs` 根目录接口，Vulkan 后端新增 `VulkanCommandEncoder.cs` / `VulkanRenderPass.cs`（dynamic rendering 用 `VkPipelineRenderingCreateInfoKHR` 无 VkRenderPass 对象，pipeline 双 handle withDepth/withoutDepth）/ New SPI-style ICommandEncoder/IRenderPass/PipelineCache abstractions, VulkanCommandEncoder/VulkanRenderPass implementations with dynamic rendering
- **阶段 2 RenderState 数据层**：新增 `Render/RenderState/` 6 个值对象（`GuiElementRenderState`/`BlitRenderState`/`TiledBlitRenderState`/`ColoredRectangleRenderState`/`GlyphBlitRenderState`/`Pip/PictureInPictureRenderState`）+ `Render/GuiRenderState.cs`（node tree + strata + NextStratum/BlurBeforeThisStratum/Up/ForEachElement/SortElements/Reset）/ 6 RenderState value objects + GuiRenderState container with node tree and strata
- **阶段 3 Pipeline 声明式系统**：新增 `Pipeline/` 12 个文件全套（`RenderPipeline` record + `Snippet` 组合 + `PipelineBuilder` + `BindGroupLayout`/`DepthStencilState`/`ColorTargetState`/`BlendFunction`/`BlendEquation`/`PipelineEnums`/`ShaderDefines`/`ShaderManager`/`VertexFormat`），`PipelineCache` 接入 `IGpuDevice.PrecompilePipeline` 运行时零 shader 编译 / Full declarative pipeline system with Snippet composition and pipeline cache
- **阶段 4 GuiRenderer 执行层**：新增 `Render/GuiRenderer.cs`（Prepare 合批 + Draw 提交 + blur 分段 FirstMeshIndexAfterBlur + PreparePip）+ `Render/StagedVertexBuffer.cs` + `Render/GpuBufferPool.cs`（fence 异步回收）+ `Render/AutoStorageIndexBuffer.cs`（QUADS 自动索引）+ `Render/IVertexConsumer.cs` / GuiRenderer with prepare/sort/blur-segmentation, StagedVertexBuffer, GpuBufferPool with fence-based recycling
- **阶段 5 submission 层迁移**：新增 `Render/GuiRenderContext.cs` 替代旧 `VulkanGuiRenderer`，把控件 DrawQuad/DrawText/DrawImage 转换为 RenderState 提交到 GuiRenderState，`_recordingStack` 支持嵌套录制 + ReplayRange cache replay；`VulkanGuiApp` 切换到 GuiRenderContext+GuiRenderer+GuiResourceManager 新路径 / GuiRenderContext replaces VulkanGuiRenderer, VulkanGuiApp switches to new path
- **阶段 6 retained mode**：`GuiRenderContext._recordingStack` 子控件 cache 和 window cache 同时活跃，控件标记 dirty 未变子树跳过 submission / Retained mode with nested recording stack and dirty-flag skip
- **阶段 8 高级特性**：新增 `Sprite/` 4 个（九宫格 `GuiSpriteScaling` Stretch/Tile/NineSlice + `.mcmeta` 解析）+ `Render/Atlas/` 7 个（`DynamicAtlasAllocator` + `GuiItemAtlas` + `ItemItemAtlas` 多 slot 共享 LoadOp=Load + `TextureStitcher` + `BlockTextureAtlas`）+ `Render/Pip/` 2 个（`PictureInPictureRenderer` + `ItemPipRenderer`，EnsureTexturesAndProjection 复用 texture 修复 descriptor pool 耗尽）+ `Shaders/core/blur.frag.glsl` + `VulkanGuiApp` blur 后处理（BeforeBlur 段 offscreen+水平+垂直 blur，AfterBlur 段 LoadOp=Load 保留模糊背景）/ Nine-slice sprite, dynamic atlas, PIP offscreen render, blur post-processing
- **字体系统重构**：新增 `Font/` 19 个文件对标原版 `client.gui.font.providers` 链（`GlyphProvider`/`FontSet`/`SheetBakedGlyph`/`GlyphStitcher`/`TtfGlyphProvider`/`BitmapGlyphProvider`/`UnihexGlyphProvider` 等），F7 GlyphFont 动态烘焙路径替代 FontAtlas fallback / Full font providers chain ported, F7 GlyphFont dynamic baking path
- **Item 3D 物品渲染**：新增 `Render/Item/` 5 个（`BakedQuad`/`CubeModel`/`VertexConsumer3D`/`ItemStackRenderState`/`ItemFeatureRenderer`），CPU 端 PoseStack transform 顶点 shader 端 Model=Identity / 3D item rendering with CPU-side PoseStack transform
- **阶段 7 Tick/Render 解耦（部分完成）**：`VulkanAppBase` 移除 FrameUpdate 事件订阅，`Run()` 只 `_window.Render += DrawFrame`，新增 `OnBeforeRun`/`OnAfterRun` 钩子供子类启动/Join Tick 线程；`VulkanGuiApp` 持有 `_latestSnapshot`（volatile）双缓冲 + `FrameTick` 事件由 Tick 线程 20tps 触发。Render 仍串行在 `_window.Run` 内（剩余优化是 Render 也独立线程）/ Tick on independent 20tps thread via OnBeforeRun hook, Render still serial in _window.Run (remaining: Render thread independence)
- **GUI 布局系统扩展**：新增 `Gui/Layouts/` 11 个文件对标原版 `client.gui.layouts.*` 全套（`GridLayout`/`LinearLayout`/`FrameLayout`/`EqualSpacingLayout`/`HeaderAndFooterLayout`/`LayoutSettings`/`Divisor`/`SpacerElement` 等）/ Full layout system ported from client.gui.layouts.*
