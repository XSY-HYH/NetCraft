...
export default class FileManager {
  public static async listFiles (path: string = '/') {
    const { data } = await serverRequest.get<ServerResponse<{ rootPath: string; entries: FileInfo[]; }>>(
  ...
  public static async readFile (path: string) {
    const { data } = await serverRequest.get<ServerResponse<{ content: string }>>(
  ...
  public static async copy (sourcePath: string, targetPath: string) {
    const { data } = await serverRequest.post<ServerResponse<boolean>>(
      '/file/copy',
      { sourcePath, targetPath }
    );
